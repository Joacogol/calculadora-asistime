...
-- Cliente «ducart-disenos» en el esquema «ducart», bucket «disenos-ducart».
create schema if not exists ducart;
grant usage on schema ducart to anon, authenticated, service_role;
alter default privileges in schema ducart
...
create or replace function ducart.tocar_actualizado() returns trigger
language plpgsql set search_path = ducart, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

create or replace function ducart.forzar_user_id() returns trigger
language plpgsql security definer set search_path = ducart, pg_temp as $$
...
create or replace function ducart.disenos_tocar_actualizado() returns trigger
language plpgsql set search_path = ducart, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

create or replace function ducart.disenos_forzar_user_id() returns trigger
language plpgsql security definer set search_path = ducart as $$
begin new.user_id := auth.uid(); return new; end $$;


-- ═══ disenos — los pedidos de pieza ══════════════════════════════════════

create table if not exists ducart.disenos (
  ...
  corrige        uuid references ducart.disenos(id) on delete set null,
  mensaje_agente text,
...
alter table ducart.disenos add column if not exists documentos     jsonb default '[]'::jsonb;
alter table ducart.disenos add column if not exists videos         jsonb default '[]'::jsonb;
alter table ducart.disenos add column if not exists adjuntos       jsonb default '[]'::jsonb;
alter table ducart.disenos add column if not exists metricas       jsonb default '{}'::jsonb;
alter table ducart.disenos add column if not exists fotos_elegidas text[] not null default '{}';
alter table ducart.disenos add column if not exists logo_socio     text;
alter table ducart.disenos add column if not exists notas          text;

comment on table  ducart.disenos is
...
comment on column ducart.disenos."copy" is
  'Texto del posteo, listo para publicar. No lleva notas ni encabezados.';
comment on column ducart.disenos.notas is
...
create index if not exists disenos_estado_idx
  on ducart.disenos (estado, creado_en);
create index if not exists disenos_user_creado_idx
  on ducart.disenos (user_id, creado_en desc);

drop trigger if exists disenos_forzar_user on ducart.disenos;
create trigger disenos_forzar_user before insert on ducart.disenos
  for each row execute function ducart.disenos_forzar_user_id();

drop trigger if exists disenos_tocar on ducart.disenos;
create trigger disenos_tocar before update on ducart.disenos
  for each row execute function ducart.disenos_tocar_actualizado();

alter table ducart.disenos enable row level security;

-- Cada uno ve lo suyo y nada más. El worker no pasa por acá: entra con la
-- service_role key, que se saltea RLS.
drop policy if exists "ver lo propio" on ducart.disenos;
create policy "ver lo propio" on ducart.disenos
  for select to authenticated using (auth.uid() = user_id and public.es_de('ducart-disenos'));

drop policy if exists "crear lo propio" on ducart.disenos;
create policy "crear lo propio" on ducart.disenos
  for insert to authenticated with check (auth.uid() is not null and public.es_de('ducart-disenos'));
...
create table if not exists ducart.fotos (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  user_id     uuid references auth.users(id) on delete set null,
...
create unique index if not exists fotos_clave_idx on ducart.fotos (clave);
create index if not exists fotos_activa_idx on ducart.fotos (activa, creado_en desc);

drop trigger if exists fotos_forzar_user on ducart.fotos;
create trigger fotos_forzar_user before insert on ducart.fotos
  for each row execute function ducart.forzar_user_id();

alter table ducart.fotos enable row level security;

-- El banco es de la marca, no de quien subió cada foto: lo ve y lo edita
-- cualquiera del equipo. Sumar sí queda a nombre de uno.
drop policy if exists "ver el banco" on ducart.fotos;
create policy "ver el banco" on ducart.fotos
  for select to authenticated using (true);

drop policy if exists "sumar al banco" on ducart.fotos;
create policy "sumar al banco" on ducart.fotos
  for insert to authenticated with check (auth.uid() = user_id and public.es_de('ducart-disenos'));

drop policy if exists "editar el banco" on ducart.fotos;
create policy "editar el banco" on ducart.fotos
  for update to authenticated using (true) with check (true);


-- ═══ cuentas_ig — el token de Instagram ══════════════════════════════════

create table if not exists ducart.cuentas_ig (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  usuario     text,
  ig_user_id  text,
...
alter table ducart.cuentas_ig enable row level security;
...
create or replace view ducart.instagram_estado
with (security_invoker = on) as
  select usuario,
         ...
         mensaje
    from ducart.cuentas_ig;

revoke all on ducart.instagram_estado from anon;
grant select on ducart.instagram_estado to authenticated, service_role;


-- ═══ publicaciones — la cola de posteo ═══════════════════════════════════

create table if not exists ducart.publicaciones (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  user_id        uuid references auth.users(id) on delete set null,
  diseno_id      uuid references ducart.disenos(id) on delete cascade,
...
create index if not exists publicaciones_cola_idx
  on ducart.publicaciones (estado, publicar_en);
create index if not exists publicaciones_user_idx
  on ducart.publicaciones (user_id, creado_en desc);
create index if not exists publicaciones_diseno_idx
  on ducart.publicaciones (diseno_id);

drop trigger if exists publicaciones_forzar_user on ducart.publicaciones;
create trigger publicaciones_forzar_user before insert on ducart.publicaciones
  for each row execute function ducart.forzar_user_id();

drop trigger if exists publicaciones_tocar on ducart.publicaciones;
create trigger publicaciones_tocar before update on ducart.publicaciones
  for each row execute function ducart.tocar_actualizado();

alter table ducart.publicaciones enable row level security;

drop policy if exists "ver lo propio pub" on ducart.publicaciones;
create policy "ver lo propio pub" on ducart.publicaciones
  for select to authenticated using (auth.uid() = user_id and public.es_de('ducart-disenos'));

drop policy if exists "programar lo propio" on ducart.publicaciones;
create policy "programar lo propio" on ducart.publicaciones
  for insert to authenticated with check (auth.uid() = user_id and public.es_de('ducart-disenos'));
...
drop policy if exists "cambiar lo no publicado" on ducart.publicaciones;
create policy "cambiar lo no publicado" on ducart.publicaciones
  for update to authenticated
  using       (auth.uid() = user_id and public.es_de('ducart-disenos') and estado in ('programado','error'))
  with check  (auth.uid() = user_id and public.es_de('ducart-disenos') and estado in ('programado','cancelado'));
...
values ('disenos-ducart', 'disenos-ducart', true)
...
-- por acá.
drop policy if exists "subir al banco · ducart" on storage.objects;
create policy "subir al banco · ducart" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-ducart' and (storage.foldername(name))[1] = 'fotos');

drop policy if exists "subir logos de socios · ducart" on storage.objects;
create policy "subir logos de socios · ducart" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-ducart' and (storage.foldername(name))[1] = 'socios');

drop policy if exists "subir mis adjuntos · ducart" on storage.objects;
create policy "subir mis adjuntos · ducart" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-ducart'
...
create table if not exists ducart.plantillas (
...
create unique index if not exists plantillas_una_publicada
  on ducart.plantillas (plantilla) where publicada;

create index if not exists plantillas_publicadas_idx
  on ducart.plantillas (plantilla, version desc);
...
create or replace function ducart.guardar_plantilla(
  p_plantilla text,
  p_html      text,
  p_contrato  jsonb,
...
) returns ducart.plantillas
language plpgsql security definer set search_path = ducart, pg_temp as $$
declare
  v_version int;
  v_fila    ducart.plantillas;
begin
  select coalesce(max(version), 0) + 1 into v_version
    from ducart.plantillas where plantilla = p_plantilla;

  insert into ducart.plantillas (plantilla, version, etiqueta, html, contrato, quien)
  values (p_plantilla, v_version, p_etiqueta, p_html, p_contrato, p_quien)
  returning * into v_fila;

  if p_publicar then
    v_fila := ducart.publicar_plantilla(p_plantilla, v_version);
  end if;
...
create or replace function ducart.publicar_plantilla(
  p_plantilla text,
  p_version   int
) returns ducart.plantillas
language plpgsql security definer set search_path = ducart, pg_temp as $$
declare v_fila ducart.plantillas;
begin
  update ducart.plantillas set publicada = false
    where plantilla = p_plantilla and publicada;

  update ducart.plantillas set publicada = true
...
alter table ducart.plantillas enable row level security;

drop policy if exists "ver lo publicado" on ducart.plantillas;
create policy "ver lo publicado" on ducart.plantillas
...
revoke execute on function ducart.guardar_plantilla(text, text, jsonb, text, text, boolean)
  from public, anon, authenticated;
revoke execute on function ducart.publicar_plantilla(text, int)
  from public, anon, authenticated;
...
create table if not exists ducart.plantilla_pedidos (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
...
alter table ducart.plantilla_pedidos
...
create index if not exists plantilla_pedidos_estado_idx
  on ducart.plantilla_pedidos (estado, creado_en);

drop trigger if exists plantilla_pedidos_tocar on ducart.plantilla_pedidos;
create trigger plantilla_pedidos_tocar before update on ducart.plantilla_pedidos
  for each row execute function ducart.tocar_actualizado();
...
alter table ducart.plantilla_pedidos enable row level security;

drop policy if exists "ver los pedidos" on ducart.plantilla_pedidos;
create policy "ver los pedidos" on ducart.plantilla_pedidos
  for select to authenticated using (true);
...
create table if not exists ducart.fotos_editadas (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
...
create index if not exists fotos_editadas_cola
  on ducart.fotos_editadas (estado, creado_en);

create or replace function ducart.fotos_editadas_tocar() returns trigger
language plpgsql set search_path = ducart, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

drop trigger if exists fotos_editadas_tocar on ducart.fotos_editadas;
create trigger fotos_editadas_tocar before update on ducart.fotos_editadas
  for each row execute function ducart.fotos_editadas_tocar();

drop trigger if exists fotos_editadas_forzar_user on ducart.fotos_editadas;
create trigger fotos_editadas_forzar_user before insert on ducart.fotos_editadas
  for each row execute function ducart.forzar_user_id();

-- RLS prendida y cada quien ve lo suyo, igual que los diseños.
alter table ducart.fotos_editadas enable row level security;

drop policy if exists "ver mis fotos editadas" on ducart.fotos_editadas;
create policy "ver mis fotos editadas" on ducart.fotos_editadas
  for select to authenticated using (user_id = auth.uid());

drop policy if exists "pedir una edición" on ducart.fotos_editadas;
create policy "pedir una edición" on ducart.fotos_editadas
...
create table if not exists ducart.motor_pedidos (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  -- lo que pidieron
...
alter table ducart.motor_pedidos add column if not exists parche    text;
alter table ducart.motor_pedidos add column if not exists evidencia jsonb;
alter table ducart.motor_pedidos add column if not exists notas     text;
alter table ducart.motor_pedidos add column if not exists metricas  jsonb default '{}'::jsonb;

alter table ducart.motor_pedidos drop constraint if exists motor_estado_valido;
alter table ducart.motor_pedidos add  constraint motor_estado_valido
  check (estado in ('anotado','generando','propuesto',
                    'en_curso','hecho','descartado','error'));

create index if not exists motor_pedidos_estado_idx
  on ducart.motor_pedidos (estado, creado_en desc);

drop trigger if exists motor_pedidos_tocar on ducart.motor_pedidos;
create trigger motor_pedidos_tocar before update on ducart.motor_pedidos
  for each row execute function ducart.tocar_actualizado();

-- Igual que las otras colas: se escribe con la service_role, desde afuera es de
-- sólo lectura y la puerta es la Edge Function.
alter table ducart.motor_pedidos enable row level security;

drop policy if exists "ver los pedidos de motor" on ducart.motor_pedidos;
create policy "ver los pedidos de motor" on ducart.motor_pedidos
  for select to authenticated using (true);
...
--     from ducart.motor_pedidos group by 1, 2 order by 1 desc, 2;
--
-- Y qué es lo que más piden, para ver si hay un patrón que se resuelva de una:
--
--   select parte, count(*) from ducart.motor_pedidos
...
--
--  ── Por qué todo está calificado con `ducart.` ──────────────────────
--
...
--  alta a otro esquema reemplazando el texto `ducart.`. Sin la calificación,
...
create table if not exists ducart.reels (
  id                  uuid primary key default gen_random_uuid(),
  creado_en           timestamptz not null default now(),
  actualizado_en      timestamptz not null default now(),
  user_id             uuid,
...
alter table ducart.reels add column if not exists clips jsonb;
alter table ducart.reels add column if not exists guion jsonb;
alter table ducart.reels alter column foto drop not null;

comment on column ducart.reels.clips is
  ...
  'no gasta créditos.';
comment on column ducart.reels.guion is
...
create index if not exists reels_estado_creado on ducart.reels (estado, creado_en);
...
alter table ducart.reels enable row level security;
...
drop trigger if exists reels_tocar on ducart.reels;
create trigger reels_tocar before update on ducart.reels
  for each row execute function ducart.tocar_actualizado();

drop trigger if exists reels_forzar_user on ducart.reels;
create trigger reels_forzar_user before insert on ducart.reels
  for each row execute function ducart.forzar_user_id();
...
alter table ducart.reels
  add column if not exists armado jsonb;

comment on column ducart.reels.armado is
...
comment on column ducart.reels.origen is
...
-- vez de taparlo.
create table if not exists ducart.correcciones (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  de          text not null,
  a           text not null,
...
comment on table ducart.correcciones is
...
alter table ducart.correcciones enable row level security;
...
drop policy if exists correcciones_sin_anon on ducart.correcciones;
create policy correcciones_sin_anon
  on ducart.correcciones
  for all
...
create table if not exists ducart.movimientos (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  ...
  diseno_id   uuid references ducart.disenos(id) on delete set null,
  detalle     text
);

create index if not exists movimientos_fecha_idx
  on ducart.movimientos (creado_en desc);
create index if not exists movimientos_tipo_idx
  on ducart.movimientos (tipo, creado_en desc);

alter table ducart.movimientos enable row level security;
...
create or replace view ducart.mi_cuenta
...
select
  ...
  (select max(creado_en) from ducart.movimientos where tipo = 'carga')
      as ultima_carga
from ducart.movimientos;

grant select on ducart.mi_cuenta to authenticated;
...
create or replace view ducart.estado_saldo
with (security_invoker = off) as
select
  saldo_usd,
  consumido_mes_usd,
...
from ducart.mi_cuenta;

grant select on ducart.estado_saldo to authenticated;
...
revoke all on ducart.movimientos from anon, authenticated;


-- Restricción adicional por membresía en TODAS las tablas del nuevo esquema.
-- Las políticas permisivas del modelo monocliente quedan acotadas sólo aquí.
do $$ declare t record; begin
 for t in select tablename from pg_tables where schemaname='ducart' loop
  execute format('alter table ducart.%I enable row level security',t.tablename);
  execute format('drop policy if exists ducart_membresia on ducart.%I',t.tablename);
  execute format('create policy ducart_membresia on ducart.%I as restrictive for all to authenticated using (public.es_de(''ducart-disenos'')) with check (public.es_de(''ducart-disenos''))',t.tablename);
 end loop;
end $$;
drop policy if exists ducart_storage_membresia on storage.objects;
create policy ducart_storage_membresia on storage.objects as restrictive for all to authenticated
 using (bucket_id <> 'disenos-ducart' or public.es_de('ducart-disenos'))
 with check (bucket_id <> 'disenos-ducart' or public.es_de('ducart-disenos'));
