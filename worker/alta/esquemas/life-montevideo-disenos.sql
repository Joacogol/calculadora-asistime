...
-- Cliente «life-montevideo-disenos» en el esquema «life_montevideo», bucket «disenos-life-montevideo».
create schema if not exists life_montevideo;
grant usage on schema life_montevideo to anon, authenticated, service_role;
alter default privileges in schema life_montevideo
...
create or replace function life_montevideo.tocar_actualizado() returns trigger
language plpgsql set search_path = life_montevideo, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

create or replace function life_montevideo.forzar_user_id() returns trigger
language plpgsql security definer set search_path = life_montevideo, pg_temp as $$
...
create or replace function life_montevideo.disenos_tocar_actualizado() returns trigger
language plpgsql set search_path = life_montevideo, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

create or replace function life_montevideo.disenos_forzar_user_id() returns trigger
language plpgsql security definer set search_path = life_montevideo as $$
begin new.user_id := auth.uid(); return new; end $$;


-- ═══ disenos — los pedidos de pieza ══════════════════════════════════════

create table if not exists life_montevideo.disenos (
  ...
  corrige        uuid references life_montevideo.disenos(id) on delete set null,
  mensaje_agente text,
...
alter table life_montevideo.disenos add column if not exists documentos     jsonb default '[]'::jsonb;
alter table life_montevideo.disenos add column if not exists videos         jsonb default '[]'::jsonb;
alter table life_montevideo.disenos add column if not exists adjuntos       jsonb default '[]'::jsonb;
alter table life_montevideo.disenos add column if not exists metricas       jsonb default '{}'::jsonb;
alter table life_montevideo.disenos add column if not exists fotos_elegidas text[] not null default '{}';
alter table life_montevideo.disenos add column if not exists logo_socio     text;
alter table life_montevideo.disenos add column if not exists notas          text;

comment on table  life_montevideo.disenos is
...
comment on column life_montevideo.disenos."copy" is
  'Texto del posteo, listo para publicar. No lleva notas ni encabezados.';
comment on column life_montevideo.disenos.notas is
...
create index if not exists disenos_estado_idx
  on life_montevideo.disenos (estado, creado_en);
create index if not exists disenos_user_creado_idx
  on life_montevideo.disenos (user_id, creado_en desc);

drop trigger if exists disenos_forzar_user on life_montevideo.disenos;
create trigger disenos_forzar_user before insert on life_montevideo.disenos
  for each row execute function life_montevideo.disenos_forzar_user_id();

drop trigger if exists disenos_tocar on life_montevideo.disenos;
create trigger disenos_tocar before update on life_montevideo.disenos
  for each row execute function life_montevideo.disenos_tocar_actualizado();

alter table life_montevideo.disenos enable row level security;

-- Cada uno ve lo suyo y nada más. El worker no pasa por acá: entra con la
-- service_role key, que se saltea RLS.
drop policy if exists "ver lo propio" on life_montevideo.disenos;
create policy "ver lo propio" on life_montevideo.disenos
  for select to authenticated using (auth.uid() = user_id and public.es_de('life-montevideo-disenos'));

drop policy if exists "crear lo propio" on life_montevideo.disenos;
create policy "crear lo propio" on life_montevideo.disenos
  for insert to authenticated with check (auth.uid() is not null and public.es_de('life-montevideo-disenos'));
...
create table if not exists life_montevideo.fotos (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  user_id     uuid references auth.users(id) on delete set null,
...
create unique index if not exists fotos_clave_idx on life_montevideo.fotos (clave);
create index if not exists fotos_activa_idx on life_montevideo.fotos (activa, creado_en desc);

drop trigger if exists fotos_forzar_user on life_montevideo.fotos;
create trigger fotos_forzar_user before insert on life_montevideo.fotos
  for each row execute function life_montevideo.forzar_user_id();

alter table life_montevideo.fotos enable row level security;

-- El banco es de la marca, no de quien subió cada foto: lo ve y lo edita
-- cualquiera del equipo. Sumar sí queda a nombre de uno.
drop policy if exists "ver el banco" on life_montevideo.fotos;
create policy "ver el banco" on life_montevideo.fotos
  for select to authenticated using (true);

drop policy if exists "sumar al banco" on life_montevideo.fotos;
create policy "sumar al banco" on life_montevideo.fotos
  for insert to authenticated with check (auth.uid() = user_id and public.es_de('life-montevideo-disenos'));

drop policy if exists "editar el banco" on life_montevideo.fotos;
create policy "editar el banco" on life_montevideo.fotos
  for update to authenticated using (true) with check (true);


-- ═══ cuentas_ig — el token de Instagram ══════════════════════════════════

create table if not exists life_montevideo.cuentas_ig (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  usuario     text,
  ig_user_id  text,
...
alter table life_montevideo.cuentas_ig enable row level security;
...
create or replace view life_montevideo.instagram_estado
with (security_invoker = on) as
  select usuario,
         ...
         mensaje
    from life_montevideo.cuentas_ig;

revoke all on life_montevideo.instagram_estado from anon;
grant select on life_montevideo.instagram_estado to authenticated, service_role;


-- ═══ publicaciones — la cola de posteo ═══════════════════════════════════

create table if not exists life_montevideo.publicaciones (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  user_id        uuid references auth.users(id) on delete set null,
  diseno_id      uuid references life_montevideo.disenos(id) on delete cascade,
...
create index if not exists publicaciones_cola_idx
  on life_montevideo.publicaciones (estado, publicar_en);
create index if not exists publicaciones_user_idx
  on life_montevideo.publicaciones (user_id, creado_en desc);
create index if not exists publicaciones_diseno_idx
  on life_montevideo.publicaciones (diseno_id);

drop trigger if exists publicaciones_forzar_user on life_montevideo.publicaciones;
create trigger publicaciones_forzar_user before insert on life_montevideo.publicaciones
  for each row execute function life_montevideo.forzar_user_id();

drop trigger if exists publicaciones_tocar on life_montevideo.publicaciones;
create trigger publicaciones_tocar before update on life_montevideo.publicaciones
  for each row execute function life_montevideo.tocar_actualizado();

alter table life_montevideo.publicaciones enable row level security;

drop policy if exists "ver lo propio pub" on life_montevideo.publicaciones;
create policy "ver lo propio pub" on life_montevideo.publicaciones
  for select to authenticated using (auth.uid() = user_id and public.es_de('life-montevideo-disenos'));

drop policy if exists "programar lo propio" on life_montevideo.publicaciones;
create policy "programar lo propio" on life_montevideo.publicaciones
  for insert to authenticated with check (auth.uid() = user_id and public.es_de('life-montevideo-disenos'));
...
drop policy if exists "cambiar lo no publicado" on life_montevideo.publicaciones;
create policy "cambiar lo no publicado" on life_montevideo.publicaciones
  for update to authenticated
  using       (auth.uid() = user_id and public.es_de('life-montevideo-disenos') and estado in ('programado','error'))
  with check  (auth.uid() = user_id and public.es_de('life-montevideo-disenos') and estado in ('programado','cancelado'));
...
values ('disenos-life-montevideo', 'disenos-life-montevideo', true)
...
-- por acá.
drop policy if exists "subir al banco · life_montevideo" on storage.objects;
create policy "subir al banco · life_montevideo" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-life-montevideo' and (storage.foldername(name))[1] = 'fotos');

drop policy if exists "subir logos de socios · life_montevideo" on storage.objects;
create policy "subir logos de socios · life_montevideo" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-life-montevideo' and (storage.foldername(name))[1] = 'socios');

drop policy if exists "subir mis adjuntos · life_montevideo" on storage.objects;
create policy "subir mis adjuntos · life_montevideo" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-life-montevideo'
...
create table if not exists life_montevideo.plantillas (
...
create unique index if not exists plantillas_una_publicada
  on life_montevideo.plantillas (plantilla) where publicada;

create index if not exists plantillas_publicadas_idx
  on life_montevideo.plantillas (plantilla, version desc);
...
create or replace function life_montevideo.guardar_plantilla(
  p_plantilla text,
  p_html      text,
  p_contrato  jsonb,
...
) returns life_montevideo.plantillas
language plpgsql security definer set search_path = life_montevideo, pg_temp as $$
declare
  v_version int;
  v_fila    life_montevideo.plantillas;
begin
  select coalesce(max(version), 0) + 1 into v_version
    from life_montevideo.plantillas where plantilla = p_plantilla;

  insert into life_montevideo.plantillas (plantilla, version, etiqueta, html, contrato, quien)
  values (p_plantilla, v_version, p_etiqueta, p_html, p_contrato, p_quien)
  returning * into v_fila;

  if p_publicar then
    v_fila := life_montevideo.publicar_plantilla(p_plantilla, v_version);
  end if;
...
create or replace function life_montevideo.publicar_plantilla(
  p_plantilla text,
  p_version   int
) returns life_montevideo.plantillas
language plpgsql security definer set search_path = life_montevideo, pg_temp as $$
declare v_fila life_montevideo.plantillas;
begin
  update life_montevideo.plantillas set publicada = false
    where plantilla = p_plantilla and publicada;

  update life_montevideo.plantillas set publicada = true
...
alter table life_montevideo.plantillas enable row level security;

drop policy if exists "ver lo publicado" on life_montevideo.plantillas;
create policy "ver lo publicado" on life_montevideo.plantillas
...
revoke execute on function life_montevideo.guardar_plantilla(text, text, jsonb, text, text, boolean)
  from public, anon, authenticated;
revoke execute on function life_montevideo.publicar_plantilla(text, int)
  from public, anon, authenticated;
...
create table if not exists life_montevideo.plantilla_pedidos (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
...
alter table life_montevideo.plantilla_pedidos
...
create index if not exists plantilla_pedidos_estado_idx
  on life_montevideo.plantilla_pedidos (estado, creado_en);

drop trigger if exists plantilla_pedidos_tocar on life_montevideo.plantilla_pedidos;
create trigger plantilla_pedidos_tocar before update on life_montevideo.plantilla_pedidos
  for each row execute function life_montevideo.tocar_actualizado();
...
alter table life_montevideo.plantilla_pedidos enable row level security;

drop policy if exists "ver los pedidos" on life_montevideo.plantilla_pedidos;
create policy "ver los pedidos" on life_montevideo.plantilla_pedidos
  for select to authenticated using (true);
...
create table if not exists life_montevideo.fotos_editadas (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
...
create index if not exists fotos_editadas_cola
  on life_montevideo.fotos_editadas (estado, creado_en);

create or replace function life_montevideo.fotos_editadas_tocar() returns trigger
language plpgsql set search_path = life_montevideo, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

drop trigger if exists fotos_editadas_tocar on life_montevideo.fotos_editadas;
create trigger fotos_editadas_tocar before update on life_montevideo.fotos_editadas
  for each row execute function life_montevideo.fotos_editadas_tocar();

drop trigger if exists fotos_editadas_forzar_user on life_montevideo.fotos_editadas;
create trigger fotos_editadas_forzar_user before insert on life_montevideo.fotos_editadas
  for each row execute function life_montevideo.forzar_user_id();

-- RLS prendida y cada quien ve lo suyo, igual que los diseños.
alter table life_montevideo.fotos_editadas enable row level security;

drop policy if exists "ver mis fotos editadas" on life_montevideo.fotos_editadas;
create policy "ver mis fotos editadas" on life_montevideo.fotos_editadas
  for select to authenticated using (user_id = auth.uid());

drop policy if exists "pedir una edición" on life_montevideo.fotos_editadas;
create policy "pedir una edición" on life_montevideo.fotos_editadas
...
create table if not exists life_montevideo.motor_pedidos (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  -- lo que pidieron
...
alter table life_montevideo.motor_pedidos add column if not exists parche    text;
alter table life_montevideo.motor_pedidos add column if not exists evidencia jsonb;
alter table life_montevideo.motor_pedidos add column if not exists notas     text;
alter table life_montevideo.motor_pedidos add column if not exists metricas  jsonb default '{}'::jsonb;

alter table life_montevideo.motor_pedidos drop constraint if exists motor_estado_valido;
alter table life_montevideo.motor_pedidos add  constraint motor_estado_valido
  check (estado in ('anotado','generando','propuesto',
                    'en_curso','hecho','descartado','error'));

create index if not exists motor_pedidos_estado_idx
  on life_montevideo.motor_pedidos (estado, creado_en desc);

drop trigger if exists motor_pedidos_tocar on life_montevideo.motor_pedidos;
create trigger motor_pedidos_tocar before update on life_montevideo.motor_pedidos
  for each row execute function life_montevideo.tocar_actualizado();

-- Igual que las otras colas: se escribe con la service_role, desde afuera es de
-- sólo lectura y la puerta es la Edge Function.
alter table life_montevideo.motor_pedidos enable row level security;

drop policy if exists "ver los pedidos de motor" on life_montevideo.motor_pedidos;
create policy "ver los pedidos de motor" on life_montevideo.motor_pedidos
  for select to authenticated using (true);
...
--     from life_montevideo.motor_pedidos group by 1, 2 order by 1 desc, 2;
--
-- Y qué es lo que más piden, para ver si hay un patrón que se resuelva de una:
--
--   select parte, count(*) from life_montevideo.motor_pedidos
...
--
--  ── Por qué todo está calificado con `life_montevideo.` ──────────────────────
--
...
--  alta a otro esquema reemplazando el texto `life_montevideo.`. Sin la calificación,
...
create table if not exists life_montevideo.reels (
  id                  uuid primary key default gen_random_uuid(),
  creado_en           timestamptz not null default now(),
  actualizado_en      timestamptz not null default now(),
  user_id             uuid,
...
alter table life_montevideo.reels add column if not exists clips jsonb;
alter table life_montevideo.reels add column if not exists guion jsonb;
alter table life_montevideo.reels alter column foto drop not null;

comment on column life_montevideo.reels.clips is
  ...
  'no gasta créditos.';
comment on column life_montevideo.reels.guion is
...
create index if not exists reels_estado_creado on life_montevideo.reels (estado, creado_en);
...
alter table life_montevideo.reels enable row level security;
...
drop trigger if exists reels_tocar on life_montevideo.reels;
create trigger reels_tocar before update on life_montevideo.reels
  for each row execute function life_montevideo.tocar_actualizado();

drop trigger if exists reels_forzar_user on life_montevideo.reels;
create trigger reels_forzar_user before insert on life_montevideo.reels
  for each row execute function life_montevideo.forzar_user_id();
...
alter table life_montevideo.reels
  add column if not exists armado jsonb;

comment on column life_montevideo.reels.armado is
...
comment on column life_montevideo.reels.origen is
...
create index if not exists reels_origen_idx
  on life_montevideo.reels (origen)
...
-- vez de taparlo.
create table if not exists life_montevideo.correcciones (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  de          text not null,
  a           text not null,
...
comment on table life_montevideo.correcciones is
...
alter table life_montevideo.correcciones enable row level security;
...
drop policy if exists correcciones_sin_anon on life_montevideo.correcciones;
create policy correcciones_sin_anon
  on life_montevideo.correcciones
  for all
...
create table if not exists life_montevideo.movimientos (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  ...
  diseno_id   uuid references life_montevideo.disenos(id) on delete set null,
  detalle     text
);

create index if not exists movimientos_fecha_idx
  on life_montevideo.movimientos (creado_en desc);
create index if not exists movimientos_tipo_idx
  on life_montevideo.movimientos (tipo, creado_en desc);

alter table life_montevideo.movimientos enable row level security;
...
create or replace view life_montevideo.mi_cuenta
...
select
  ...
  (select max(creado_en) from life_montevideo.movimientos where tipo = 'carga')
      as ultima_carga
from life_montevideo.movimientos;

grant select on life_montevideo.mi_cuenta to authenticated;
...
create or replace view life_montevideo.estado_saldo
with (security_invoker = off) as
select
  saldo_usd,
  consumido_mes_usd,
...
from life_montevideo.mi_cuenta;

grant select on life_montevideo.estado_saldo to authenticated;
...
revoke all on life_montevideo.movimientos from anon, authenticated;
