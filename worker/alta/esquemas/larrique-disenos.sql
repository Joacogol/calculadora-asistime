...
-- Cliente «larrique-disenos» en el esquema «larrique», bucket «disenos-larrique».
create schema if not exists larrique;
grant usage on schema larrique to anon, authenticated, service_role;
alter default privileges in schema larrique
...
create or replace function larrique.tocar_actualizado() returns trigger
language plpgsql set search_path = larrique, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

create or replace function larrique.forzar_user_id() returns trigger
language plpgsql security definer set search_path = larrique, pg_temp as $$
...
create or replace function larrique.disenos_tocar_actualizado() returns trigger
language plpgsql set search_path = larrique, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

create or replace function larrique.disenos_forzar_user_id() returns trigger
language plpgsql security definer set search_path = larrique as $$
begin new.user_id := auth.uid(); return new; end $$;


-- ═══ disenos — los pedidos de pieza ══════════════════════════════════════

create table if not exists larrique.disenos (
  ...
  corrige        uuid references larrique.disenos(id) on delete set null,
  mensaje_agente text,
...
alter table larrique.disenos add column if not exists documentos     jsonb default '[]'::jsonb;
alter table larrique.disenos add column if not exists videos         jsonb default '[]'::jsonb;
alter table larrique.disenos add column if not exists adjuntos       jsonb default '[]'::jsonb;
alter table larrique.disenos add column if not exists metricas       jsonb default '{}'::jsonb;
alter table larrique.disenos add column if not exists fotos_elegidas text[] not null default '{}';
alter table larrique.disenos add column if not exists logo_socio     text;
alter table larrique.disenos add column if not exists notas          text;

comment on table  larrique.disenos is
...
comment on column larrique.disenos."copy" is
  'Texto del posteo, listo para publicar. No lleva notas ni encabezados.';
comment on column larrique.disenos.notas is
...
create index if not exists disenos_estado_idx
  on larrique.disenos (estado, creado_en);
create index if not exists disenos_user_creado_idx
  on larrique.disenos (user_id, creado_en desc);

drop trigger if exists disenos_forzar_user on larrique.disenos;
create trigger disenos_forzar_user before insert on larrique.disenos
  for each row execute function larrique.disenos_forzar_user_id();

drop trigger if exists disenos_tocar on larrique.disenos;
create trigger disenos_tocar before update on larrique.disenos
  for each row execute function larrique.disenos_tocar_actualizado();

alter table larrique.disenos enable row level security;

-- Cada uno ve lo suyo y nada más. El worker no pasa por acá: entra con la
-- service_role key, que se saltea RLS.
drop policy if exists "ver lo propio" on larrique.disenos;
create policy "ver lo propio" on larrique.disenos
  for select to authenticated using (auth.uid() = user_id and public.es_de('larrique-disenos'));

drop policy if exists "crear lo propio" on larrique.disenos;
create policy "crear lo propio" on larrique.disenos
  for insert to authenticated with check (auth.uid() is not null and public.es_de('larrique-disenos'));
...
create table if not exists larrique.fotos (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  user_id     uuid references auth.users(id) on delete set null,
...
create unique index if not exists fotos_clave_idx on larrique.fotos (clave);
create index if not exists fotos_activa_idx on larrique.fotos (activa, creado_en desc);

drop trigger if exists fotos_forzar_user on larrique.fotos;
create trigger fotos_forzar_user before insert on larrique.fotos
  for each row execute function larrique.forzar_user_id();

alter table larrique.fotos enable row level security;

-- El banco es de la marca, no de quien subió cada foto: lo ve y lo edita
-- cualquiera del equipo. Sumar sí queda a nombre de uno.
drop policy if exists "ver el banco" on larrique.fotos;
create policy "ver el banco" on larrique.fotos
  for select to authenticated using (true);

drop policy if exists "sumar al banco" on larrique.fotos;
create policy "sumar al banco" on larrique.fotos
  for insert to authenticated with check (auth.uid() = user_id and public.es_de('larrique-disenos'));

drop policy if exists "editar el banco" on larrique.fotos;
create policy "editar el banco" on larrique.fotos
  for update to authenticated using (true) with check (true);


-- ═══ cuentas_ig — el token de Instagram ══════════════════════════════════

create table if not exists larrique.cuentas_ig (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  usuario     text,
  ig_user_id  text,
...
alter table larrique.cuentas_ig enable row level security;
...
create or replace view larrique.instagram_estado
with (security_invoker = on) as
  select usuario,
         ...
         mensaje
    from larrique.cuentas_ig;

revoke all on larrique.instagram_estado from anon;
grant select on larrique.instagram_estado to authenticated, service_role;


-- ═══ publicaciones — la cola de posteo ═══════════════════════════════════

create table if not exists larrique.publicaciones (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  user_id        uuid references auth.users(id) on delete set null,
  diseno_id      uuid references larrique.disenos(id) on delete cascade,
...
create index if not exists publicaciones_cola_idx
  on larrique.publicaciones (estado, publicar_en);
create index if not exists publicaciones_user_idx
  on larrique.publicaciones (user_id, creado_en desc);
create index if not exists publicaciones_diseno_idx
  on larrique.publicaciones (diseno_id);

drop trigger if exists publicaciones_forzar_user on larrique.publicaciones;
create trigger publicaciones_forzar_user before insert on larrique.publicaciones
  for each row execute function larrique.forzar_user_id();

drop trigger if exists publicaciones_tocar on larrique.publicaciones;
create trigger publicaciones_tocar before update on larrique.publicaciones
  for each row execute function larrique.tocar_actualizado();

alter table larrique.publicaciones enable row level security;

drop policy if exists "ver lo propio pub" on larrique.publicaciones;
create policy "ver lo propio pub" on larrique.publicaciones
  for select to authenticated using (auth.uid() = user_id and public.es_de('larrique-disenos'));

drop policy if exists "programar lo propio" on larrique.publicaciones;
create policy "programar lo propio" on larrique.publicaciones
  for insert to authenticated with check (auth.uid() = user_id and public.es_de('larrique-disenos'));
...
drop policy if exists "cambiar lo no publicado" on larrique.publicaciones;
create policy "cambiar lo no publicado" on larrique.publicaciones
  for update to authenticated
  using       (auth.uid() = user_id and public.es_de('larrique-disenos') and estado in ('programado','error'))
  with check  (auth.uid() = user_id and public.es_de('larrique-disenos') and estado in ('programado','cancelado'));
...
values ('disenos-larrique', 'disenos-larrique', true)
...
-- por acá.
drop policy if exists "subir al banco · larrique" on storage.objects;
create policy "subir al banco · larrique" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-larrique' and (storage.foldername(name))[1] = 'fotos');

drop policy if exists "subir logos de socios · larrique" on storage.objects;
create policy "subir logos de socios · larrique" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-larrique' and (storage.foldername(name))[1] = 'socios');

drop policy if exists "subir mis adjuntos · larrique" on storage.objects;
create policy "subir mis adjuntos · larrique" on storage.objects
  for insert to authenticated with check (
    bucket_id = 'disenos-larrique'
...
create table if not exists larrique.plantillas (
...
create unique index if not exists plantillas_una_publicada
  on larrique.plantillas (plantilla) where publicada;

create index if not exists plantillas_publicadas_idx
  on larrique.plantillas (plantilla, version desc);
...
create or replace function larrique.guardar_plantilla(
  p_plantilla text,
  p_html      text,
  p_contrato  jsonb,
...
) returns larrique.plantillas
language plpgsql security definer set search_path = larrique, pg_temp as $$
declare
  v_version int;
  v_fila    larrique.plantillas;
begin
  select coalesce(max(version), 0) + 1 into v_version
    from larrique.plantillas where plantilla = p_plantilla;

  insert into larrique.plantillas (plantilla, version, etiqueta, html, contrato, quien)
  values (p_plantilla, v_version, p_etiqueta, p_html, p_contrato, p_quien)
  returning * into v_fila;

  if p_publicar then
    v_fila := larrique.publicar_plantilla(p_plantilla, v_version);
  end if;
...
create or replace function larrique.publicar_plantilla(
  p_plantilla text,
  p_version   int
) returns larrique.plantillas
language plpgsql security definer set search_path = larrique, pg_temp as $$
declare v_fila larrique.plantillas;
begin
  update larrique.plantillas set publicada = false
    where plantilla = p_plantilla and publicada;

  update larrique.plantillas set publicada = true
...
alter table larrique.plantillas enable row level security;

drop policy if exists "ver lo publicado" on larrique.plantillas;
create policy "ver lo publicado" on larrique.plantillas
...
revoke execute on function larrique.guardar_plantilla(text, text, jsonb, text, text, boolean)
  from public, anon, authenticated;
revoke execute on function larrique.publicar_plantilla(text, int)
  from public, anon, authenticated;
...
create table if not exists larrique.plantilla_pedidos (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
...
alter table larrique.plantilla_pedidos
...
create index if not exists plantilla_pedidos_estado_idx
  on larrique.plantilla_pedidos (estado, creado_en);

drop trigger if exists plantilla_pedidos_tocar on larrique.plantilla_pedidos;
create trigger plantilla_pedidos_tocar before update on larrique.plantilla_pedidos
  for each row execute function larrique.tocar_actualizado();
...
alter table larrique.plantilla_pedidos enable row level security;

drop policy if exists "ver los pedidos" on larrique.plantilla_pedidos;
create policy "ver los pedidos" on larrique.plantilla_pedidos
  for select to authenticated using (true);
...
create table if not exists larrique.fotos_editadas (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
...
create index if not exists fotos_editadas_cola
  on larrique.fotos_editadas (estado, creado_en);

create or replace function larrique.fotos_editadas_tocar() returns trigger
language plpgsql set search_path = larrique, pg_temp as $$
begin new.actualizado_en := now(); return new; end $$;

drop trigger if exists fotos_editadas_tocar on larrique.fotos_editadas;
create trigger fotos_editadas_tocar before update on larrique.fotos_editadas
  for each row execute function larrique.fotos_editadas_tocar();

drop trigger if exists fotos_editadas_forzar_user on larrique.fotos_editadas;
create trigger fotos_editadas_forzar_user before insert on larrique.fotos_editadas
  for each row execute function larrique.forzar_user_id();

-- RLS prendida y cada quien ve lo suyo, igual que los diseños.
alter table larrique.fotos_editadas enable row level security;

drop policy if exists "ver mis fotos editadas" on larrique.fotos_editadas;
create policy "ver mis fotos editadas" on larrique.fotos_editadas
  for select to authenticated using (user_id = auth.uid());

drop policy if exists "pedir una edición" on larrique.fotos_editadas;
create policy "pedir una edición" on larrique.fotos_editadas
...
create table if not exists larrique.motor_pedidos (
  id             uuid primary key default gen_random_uuid(),
  creado_en      timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  -- lo que pidieron
...
alter table larrique.motor_pedidos add column if not exists parche    text;
alter table larrique.motor_pedidos add column if not exists evidencia jsonb;
alter table larrique.motor_pedidos add column if not exists notas     text;
alter table larrique.motor_pedidos add column if not exists metricas  jsonb default '{}'::jsonb;

alter table larrique.motor_pedidos drop constraint if exists motor_estado_valido;
alter table larrique.motor_pedidos add  constraint motor_estado_valido
  check (estado in ('anotado','generando','propuesto',
                    'en_curso','hecho','descartado','error'));

create index if not exists motor_pedidos_estado_idx
  on larrique.motor_pedidos (estado, creado_en desc);

drop trigger if exists motor_pedidos_tocar on larrique.motor_pedidos;
create trigger motor_pedidos_tocar before update on larrique.motor_pedidos
  for each row execute function larrique.tocar_actualizado();

-- Igual que las otras colas: se escribe con la service_role, desde afuera es de
-- sólo lectura y la puerta es la Edge Function.
alter table larrique.motor_pedidos enable row level security;

drop policy if exists "ver los pedidos de motor" on larrique.motor_pedidos;
create policy "ver los pedidos de motor" on larrique.motor_pedidos
  for select to authenticated using (true);
...
--     from larrique.motor_pedidos group by 1, 2 order by 1 desc, 2;
--
-- Y qué es lo que más piden, para ver si hay un patrón que se resuelva de una:
--
--   select parte, count(*) from larrique.motor_pedidos
...
--
--  ── Por qué todo está calificado con `larrique.` ──────────────────────
--
...
--  alta a otro esquema reemplazando el texto `larrique.`. Sin la calificación,
...
create table if not exists larrique.reels (
  id                  uuid primary key default gen_random_uuid(),
  creado_en           timestamptz not null default now(),
  actualizado_en      timestamptz not null default now(),
  user_id             uuid,
...
alter table larrique.reels add column if not exists clips jsonb;
alter table larrique.reels add column if not exists guion jsonb;
alter table larrique.reels alter column foto drop not null;

comment on column larrique.reels.clips is
  ...
  'no gasta créditos.';
comment on column larrique.reels.guion is
...
create index if not exists reels_estado_creado on larrique.reels (estado, creado_en);
...
alter table larrique.reels enable row level security;
...
drop trigger if exists reels_tocar on larrique.reels;
create trigger reels_tocar before update on larrique.reels
  for each row execute function larrique.tocar_actualizado();

drop trigger if exists reels_forzar_user on larrique.reels;
create trigger reels_forzar_user before insert on larrique.reels
  for each row execute function larrique.forzar_user_id();
...
alter table larrique.reels
  add column if not exists armado jsonb;

comment on column larrique.reels.armado is
...
comment on column larrique.reels.origen is
...
-- vez de taparlo.
create table if not exists larrique.correcciones (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  de          text not null,
  a           text not null,
...
comment on table larrique.correcciones is
...
alter table larrique.correcciones enable row level security;
...
drop policy if exists correcciones_sin_anon on larrique.correcciones;
create policy correcciones_sin_anon
  on larrique.correcciones
  for all
...
create table if not exists larrique.movimientos (
  id          uuid primary key default gen_random_uuid(),
  creado_en   timestamptz not null default now(),
  ...
  diseno_id   uuid references larrique.disenos(id) on delete set null,
  detalle     text
);

create index if not exists movimientos_fecha_idx
  on larrique.movimientos (creado_en desc);
create index if not exists movimientos_tipo_idx
  on larrique.movimientos (tipo, creado_en desc);

alter table larrique.movimientos enable row level security;
...
create or replace view larrique.mi_cuenta
...
select
  ...
  (select max(creado_en) from larrique.movimientos where tipo = 'carga')
      as ultima_carga
from larrique.movimientos;

grant select on larrique.mi_cuenta to authenticated;
...
create or replace view larrique.estado_saldo
with (security_invoker = off) as
select
  saldo_usd,
  consumido_mes_usd,
...
from larrique.mi_cuenta;

grant select on larrique.estado_saldo to authenticated;
...
revoke all on larrique.movimientos from anon, authenticated;
