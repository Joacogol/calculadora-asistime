...
Deno.serve(async (req) => {
  ...
  // Con qué se sella la elección de proveedor. Era la propia `API_CLAVE`, que
  // está escrita en claro en el código de la tool: cualquiera que la leyera
  // podía fabricar un sello y saltearse la pregunta. La `service_role` no sale
  // de acá, y la marca adentro hace que un sello de un cliente no valga en la
  // base de otro.
  const sellador = `${llave}:${quien.marca}`;
  ...
  if (req.method === "GET") {
    ...
    if (url0.searchParams.get("opciones")) {
      return json({ proveedores: await opcionesDeProveedor(sellador) });
    }
  ...
  if (!clips.length) {
    ...
    if (!p) return json(await preguntaProveedor(sellador));
    proveedor = await proveedorSellado(p, sellador);
    if (!proveedor) {
      return json(await preguntaProveedor(sellador, {
