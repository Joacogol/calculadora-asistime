...
Deno.serve(async (req) => {
  ...
  if (esFoto) {
    ...
    if (vid) {
      ...
      const upv = await fetch(`${base}/storage/v1/object/${bucket}/${rutaVid}`, {
        method: "POST",
        headers: {
          apikey: llave,
          Authorization: `Bearer ${llave}`,
      ...
      const nuestroVid =
        `${base}/storage/v1/object/public/${bucket}/${rutaVid}`;
    ...
    if (fmt) {
      ...
      const up = await fetch(`${base}/storage/v1/object/${bucket}/${rutaFoto}`, {
        method: "POST",
        headers: {
          apikey: llave,
          Authorization: `Bearer ${llave}`,
      ...
      const nuestra = `${base}/storage/v1/object/public/${bucket}/${rutaFoto}`;
