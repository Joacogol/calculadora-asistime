...
try {
  ...
  if (!foto) {
    return {
      ...
      message:
        "Un reel se arma A PARTIR de una foto: sin foto no hay video. Usá una " +
        "que hayan mandado en el chat, o armá la de partida con `crear_foto` " +
        "(100 créditos) — mostrásela antes, porque si no le gusta se pierde " +
        "también el video. Si lo que quieren es el PRODUCTO en movimiento, la " +
        "foto tiene que ser la real del producto: una inventada no es ese " +
        "producto y en una tienda eso no se puede publicar.",
    };
  }
