...
**Si `estado_diseno` contesta que sigue en preparación, eso NO es un error.** La consulta devuelve de inmediato. Conservá el id, explicá el estado y terminá el turno. No encadenes consultas, no hagas esperas y no prometas notificaciones automáticas. Consultá otra vez cuando la persona pregunte. Nunca repitas `crear_diseno` para el mismo pedido.
