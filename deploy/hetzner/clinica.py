"""Procesa diseños y fotos de Clínica Preventiva, uno nuevo de cada tipo por ciclo."""
import asyncio
import json
import os
...
MARCA = 'clinica-preventiva-disenos'
d = json.loads(Path('/run/credenciales.json').read_text())
c = json.loads(Path('/run/clinica.json').read_text())
os.environ['ASISTIME_CLAVE_CLINICA'] = c['ASISTIME_CLAVE_CLINICA']
...
config.VERSION = 'hetzner-clinica-v1-20260910'
cli = Cliente(MARCA, 'https://jejohzzxxnhktdxpdqpy.supabase.co', c['SUPABASE_KEY_CLINICA'],
              bucket='disenos', nombre='Clínica Preventiva')
